...
object FrequencyAnalyzer {
  ...
  val alphabetsPlusSpace: String = "ABCDEFGHIJKLMNOPQRSTUVWXYZ " + "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    .toLowerCase()
  ...
  def shiftDistributionValues(map: scala.collection.Map[Char, Double],
                              rightShift: Int): Map[Char, Double] = {
    ...
    val newValues = values.slice(values.length - rightShift, values.length) ++ values.slice(
      0,
      values.length - rightShift)
  ...
  def processFileTrigraphs(bs: BufferedSource, fm: FrequencyMap[String]): FrequencyMap[String] = {
    val linesIt = bs.getLines()
    while (linesIt.hasNext) {
      val line      = linesIt.next
  ...
  def filterOutNonUpperCase(fm: FrequencyMap[Char]): FrequencyMap[Char] = {
    fm.getKeyList.foreach(x =>
      if (UPPERALPHABET.indexOf(x) == -1) {
        fm.removeKey(x)
    })
    fm
  }
  ...
  def getConsonantClusterList(pathAndFile: java.io.File, numberOfConsonants: Int): List[String] = {
  ...
  def getWordsOfLength(pathAndFile: java.io.File, numberOfLetters: Int): List[String] = {
    val bs      = loadFile(pathAndFile)
    val buffer  = new ListBuffer[String]()
    val linesIt = bs.getLines()
    while (linesIt.hasNext) {
  ...
  def convertToCharacterMap(source: String, dest: String): mutable.HashMap[Char, Char] = {
    buildCharacterMap(source.toCharArray.toList, dest.toCharArray.toList)
  }

  // todo write test
  def buildCharacterMap(sourceChars: List[Char],
                        destChars: List[Char]): mutable.HashMap[Char, Char] = {
  ...
  /**
    ...
    * @return
    */
  def extractMapping(charMap: mutable.HashMap[Char, Char],
                     word: String): mutable.HashMap[Char, Char] = {
    ...
    (0 until word.length).foreach(ii =>
      mapping.put(invertedMapping(word.charAt(ii)), word.charAt(ii)))
    mapping
  }
  ...
  def extractNonLetterPositionList(text: String): List[(Int, Char)] = {
    ...
    (0 until text.length).foreach(pos => {
      if (UPPERALPHABET.indexOf(text.charAt(pos).toUpper) == -1) {
  ...
  def insertNonLetterPositionList(text: String,
                                  nonLetterPositionList: List[(Int, Char)]): String = {
    val buf = new StringBuffer
    buf.append(text)
  ...
  def completeLetterList(cipherFMKeys: List[Char], lettersByFrequency: List[Char]): List[Char] = {
    ...
    lettersByFrequency.foreach(c =>
      if (!cipherFMKeys.contains(c)) {
        buf.append(c)
    })
    buf.toList
  }
  ...
  def extractWordList(file: java.io.File, wordLength: Int): List[String] = {
    ...
    try {
      ...
      case _: java.nio.charset.MalformedInputException =>
        println(" java.nio.charset.MalformedInputException weirdness: " + line)
    }
    hs.toList
  }
