...
class SubstitutionCipherSolverTest extends FlatSpec {
  ...
  "SubstitutionCipherSolver" should "be able to decrypt Caesarean Ciphers" in {
    ...
    assert(
      getBestCandidate("FRZDUGV GLH PDQB WLPHV EHIRUH WKHLU GHDWKV")
        == "COWARDS DIE MANY TIMES BEFORE THEIR DEATHS")
    assert(
      getBestCandidate("WKH HYLO WKDW PHQ GR OLYHV DIWHU WKHP")
        == "THE EVIL THAT MEN DO LIVES AFTER THEM")
    assert(
      getBestCandidate(
        "ESPCP TD L ETOP TYP ESP LQQLTCD ZQ XPY HSTNS ELVPY LE ESP QWZZO WPLOD ZY EZ QZCEFYP")
        == "THERE IS A TIDE INE THE AFFAIRS OF MEN WHICH TAKEN AT THE FLOOD LEADS ON TO FORTUNE")

    assert(
      getBestCandidate(
        "CQSOB KOHSF WG PZIS PSQOIGS RWFH DOFHWQZSG WB HVS KOHSF FSTZSQH GIBZWUVH PIH HVS KOHSF OPGCFPG FSR OBR MSZZCK HVS UFSSBG OBR PZISG HVOH OFS ZSTH AOYS HVS RSSD PZIS CQSOB")
    ...
    assert(
      getBestCandidate("VXMDUJA JARCQVNCRL LXDUM KN LXWBRMNANM ANVJRWMNA JARCQVNCRL")
        == "MODULAR ARITHMETIC COULD BE CONSIDERED REMAINDER ARITHMETIC")

    assert(
      getBestCandidate("MZVYDIB DN OJ OCZ HDIY RCVO ZSZMXDNZ DN OJ OCZ WJYT")
        == "READING IS TO THE MIND WHAT EXERCISE IS TO THE BODY")

    assert(
      getBestCandidate(
        "SNHPJQ NX F MJFAD XNQAJW BMNYJ RJYFQQNH JQJRJSY NY NX RFLSJYNH YFPJX F MNLM UTQNXM FSI ITJX STY YFWSNXM TW WZXY JFXNQD")
  ...
  def test_FBI_Cipher_Challenge_2008() {
    println(
      "At this point we can see the failure using substitution based decryption on random alphabet ciphers")
    val textToDecipher =
  ...
  def test_Edgar_Allan_Poe_Goldbug_Cipher() {
    val textToDecipher =
      "GE JEASGDXV, ZIJ GL MW, LAAM, XZY ZMLWHFZEK EJLVDXW KWKE TX LBR ATGH LBMX AANU BAI VSMUKKSS PWN VLWK AGH GNUMK WDLNZWEG JNBXW OAEG ENWB ZWMGY MO MLW WNBX MW AL PNFDCFPKH WZKEX HSSF XKIYAHUL. MK NUM YEXDM WBXY SBC HV WYX PHWKGNAMCUK?"
    println(getBestCandidate(textToDecipher))
  }
}
